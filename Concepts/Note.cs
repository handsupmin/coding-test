...
형변환이 가능하면 형변환을 수행하고, 그렇지 않으면 null 값을 대입
참조형 변수에 대해서만 적용할 수 있으며, 참조형 타입으로만 체크가 가능하다.

IS 연산자

형변환이 가능한 여부를 불린형으로 결과값을 반환
형변환된 인스턴스가 필요하면 AS 연산자, 그렇지 않으면 IS 연산자를 사용
대부분 시나리오에서는 AS 연산자보다 IS 연산자를 먼저 사용해서 객체의 값을 할당하는 것이 더 적합
*/